...
(function ($) {
	...
	// For elements that trigger a header background switch
	// The function are defined in custom.js
	
		$(".header-switch-onhover").on({
    		mouseenter: function() {
       			$.debounce(400, false, switchHeader(this));  
       			checkforDoubleHeader();
    		},
    	});
    	...
    	$(".bibliography").on({
    		mouseleave: function() {
       			$.debounce(400, false, switchHeader('default')); 
   				checkforDoubleHeader();
 			}
    	});
	...
	$(document).on('click', '.js-menu-toggle', function (){
		...
		if ( $('body').hasClass('menu--open') ) {
			$('body').removeClass('menu--open');
			setTimeout( function() {
				$('.fixed-top-buttons').css('visibility', 'visible');
			}, 150);
		}
		...
		// If not open
			...
			$('.fixed-top-buttons').css('visibility', 'hidden');
		}
	});
